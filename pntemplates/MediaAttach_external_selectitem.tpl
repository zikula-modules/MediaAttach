...
<div class="z-formrow">
    ...
    <select id="{$baseID}_did" name="{$baseID}_did" onchange="mediaAttach.itemSelector.onParamChanged()">
        ...
        <option value="{$currentdef.did}"{if $definitionid eq $currentdef.did} selected="selected"{/if}>{zmodgetinfo modname=$currentdef.modname info="displayname"} ({$currentdef.modname})&nbsp;</option>
        {/foreach}
    </select>
</div>
